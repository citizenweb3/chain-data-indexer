...
## 1. Real per-block transaction, note, and nullifier population by parsing block bytes
...
## 2. Wire `SyncState` paging into the runner for richer per-block context
...
## 3. Account state diff tracking once decoding is settled
...
## 4. Resolve the seven open questions in `docs/data-model.md`
...
## 5. Maintained counter table for `/api/v1/stats`
...
## 8. Multi-RPC failover in `MidenRpcClient`
