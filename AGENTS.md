...
- `src/sink/postgres.ts` exports `processBlock`, `processBatch`, and row types; it inserts block bundles and optional full-scope rows into Postgres, derives `block_hash` as `SHA-256(raw_block_bytes)` when bytes are non-empty or a fixed header-derived SHA-256 fallback when raw block bytes are missing/empty, and updates progress with `GREATEST`.
