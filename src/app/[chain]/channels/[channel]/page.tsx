...
export default async function ChannelDetailPage({
  ...
  const channelRow = channelsResult.data.find((c) => c.channel_id_src === decoded);
    ...
    <main className="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-8 px-6 py-12">
      <header className="flex flex-col gap-2">
        <Link
          ...
          className="font-sfpro hover:text-highlight text-xs tracking-wide text-white/50 uppercase"
        >
          ‹ back to dashboard
        </Link>
        ...
        <p className="font-sfpro text-sm text-white/60">
          ...
          {channelRow.channel_id_dst ? ` · counterparty ${channelRow.channel_id_dst}` : ''}
          {channelRow.counterparty_chain_id ? ` (${channelRow.counterparty_chain_id})` : ''}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="flex flex-col gap-3">
          <Subtitle>Native volume · {periodLabel[period]}</Subtitle>
          <PendingSwitch fallback={<LoadingBlock height="h-32" />}>
            <Card>
              <CardValue>{formatUsd(channelRow.volume_native[period])}</CardValue>
              <CardSubtext>{channelRow.native_symbol}</CardSubtext>
            </Card>
          </PendingSwitch>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <PendingSwitch fallback={<LoadingBlock height="h-32" />}>
            <Card>
              ...
              <CoverageDisclosure
                status={channelRow.coverage[period]}
                sources={channelsResult.sources}
                compact
                className="mt-2"
              />
            </Card>
      ...
      <div className="flex flex-col gap-3">
        ...
        >
          <PendingSwitch fallback={<LoadingBlock height="h-96" label="loading packets" />}>
