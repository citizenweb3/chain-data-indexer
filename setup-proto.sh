...
# -----------------------------------------------------------------------------
# Purpose:
#   Fetch and stage all required .proto definitions used by the indexer.
#   This script pulls proto files from Buf registries and upstream Git repos,
#   then applies a few local patches/stubs needed for smooth compilation.
#
# Requirements:
...
#
# Notes:
#   - The script removes the local ./protos directory first.
#   - Some sections create additional directories under ./proto (singular) to
#     drop overrides or patched files intentionally. This mirrors the original
#     layout used by some Cosmos projects.
# -----------------------------------------------------------------------------

# 1) Check that the buf CLI is installed
if ! command -v buf &> /dev/null; then
  echo "Error: buf CLI not found. Install it with: npm install -g @bufbuild/buf"
  exit 1
fi
...
# 2) Create a fresh structure
echo "✨ Creating protos/ directory…"
mkdir protos

echo "✨ Exporting cosmos-sdk…"
buf export buf.build/cosmos/cosmos-sdk --output protos

echo "✨ Exporting cosmos-sdk-lsm…"
...
# Pull CosmWasm (wasmd) protos for wasm v1
...
echo "✨ Cleaning old Gaia liquid protos…"
rm -rf protos/gaia/liquid

echo "✨ Cloning cosmos/gaia (using main for simplicity)…"
git clone https://github.com/cosmos/gaia.git tmp

echo "✨ Preparing directory for liquid…"
mkdir -p protos/gaia/liquid/v1beta1

echo "✨ Copying .proto from x/liquid/types…"
cp tmp/proto/gaia/liquid/v1beta1/*.proto protos/gaia/liquid/v1beta1/

echo "  Copied files:"
ls protos/gaia/liquid/v1beta1

echo "✨ Cleaning temporary clone…"
rm -rf tmp

echo "✅ Gaia liquid protos staged at protos/gaia/liquid/v1beta1"



# --- Additional stubs / patches ---------------------------------------------

# (Your existing proto fetching code would run before this point.)

echo "✨ Providing a stub for google/protobuf/descriptor.proto…"

# Create the directory if needed
mkdir -p protos/google/protobuf

# Overwrite the file with a minimal stub to satisfy imports in Cosmos protos
cat > protos/google/protobuf/descriptor.proto << 'EOF'
syntax = "proto3";
package google.protobuf;

// Minimal stub to satisfy imports in Cosmos protos
...
# Patch interchain_security Query proto with a fully overridden file.
# NOTE: This intentionally writes to ./proto (singular) to mirror upstream
#       include paths used during builds. Do not change unless you also
#       update downstream includes.

echo "Patching interchain_security Query proto (full override)…"
